...
export default function TwoStepVerificationPage() {
    const twoFactorSettings = useSettingsStore(state => state.twoFactorSettings)
    const fetchTwoFactorSettings = useSettingsStore(state => state.fetchTwoFactorSettings)
    const sendTwoFactorCode = useSettingsStore(state => state.sendTwoFactorCode)
    ...
    useEffect(() => {
        void fetchTwoFactorSettings().catch((error) => {
            toast.error(error instanceof Error ? error.message : 'Unable to load two-factor settings.')
        })
    ...
    const handleVerify = async () => {
        try {
            ...
            toast.error(error instanceof Error ? error.message : 'Unable to update two-step verification.')
        }
    }
                        ...
                        <div className="flex items-center justify-between p-6 rounded-b-2xl">
                            <div className="space-y-1 text-left">
                                ...
                                <p className="text-sm text-cyan-400 font-medium">
                                    Email to {twoFactorSettings?.deliveryLabel || 'Unavailable'}
                                </p>
                            </div>
                        </div>
                    </div>
